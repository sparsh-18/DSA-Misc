...
class Solution
{
    ...
    long verticallyDownBST(Node root,int target)
    {
        ...
        findsum(targetNode, 0, target);
        
        return ans;
    }
    
    private void findsum(Node t, int pos, int target) {
        if(t == null)
            return;
        
        if(t.data != target && pos == 0)
            ans += t.data;
        
        findsum(t.left, pos-1, target);
        findsum(t.right, pos+1, target);
    }
    ...
    private Node getTargetNode(Node root, int target) {
        ...
        Node leftChild = getTargetNode(root.left, target);
